...
## Overview
Conventions and patterns for writing and running unit tests in the Nverse Rust project.
...
- **Tests that require elevated privileges** (e.g., raw packet capture via pcap) must be marked `#[ignore]` so they are skipped in normal runs:
  ```rust
  #[test]
...
- **Assertion macros:**
  ...
  - `assert_matches!(value, pattern)` — pattern match (requires `#![feature(assert_matches)]` on stable, or import from `std::assert_matches`)

## Commands / Examples

```bash
# Run all lib unit tests (recommended — avoids macroquad/pcap linker issues)
cargo test --lib
