...
## Overview
Notes on using macroquad 0.4 for 2D/3D rendering in Nverse, including the main loop pattern, 3D drawing primitives, 2D overlay, Linux system library requirements, and headless testing strategy.
...
- **Linux system library requirements** — macroquad requires the following native libraries to link and run. They must be installed on the host:
...
- **Headless / CI testing** — macroquad cannot render without a display. Do NOT include rendering code in unit tests. Use `cargo test --lib` to compile and run only the library crate; this excludes the binary target and avoids linking macroquad altogether. All rendering logic should be isolated in the `rdr` module and tested via integration or manual inspection only.
...
# Run the application (requires a display; root for pcap)
sudo ./target/release/nverse
```

## Coordinate System

macroquad uses a right-handed coordinate system. For Nverse's IP-to-position mapping:
...
IPs in the same /16 or /24 subnet cluster spatially, producing the starfield effect.

## Common Pitfalls

- **`Camera3D.fovy` is in DEGREES, not radians.** Passing `.to_radians()` converts 60° to ~1.047, which macroquad interprets as ~1 degree — causing extreme zoom (60x magnification). The local star filled the screen and a line artifact appeared. Fix: pass degrees directly.
  ```rust
  // WRONG:
