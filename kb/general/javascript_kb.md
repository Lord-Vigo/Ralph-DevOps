...
Use ES modules for all JavaScript in this project. Each file is a module:
...
// Usage
try {
  const text = await loadFile('photos/abstract/photo/info.md');
} catch (err) {
...
- `Promise.allSettled`: waits for all, collects results including rejections — use when partial failure is acceptable (e.g., loading a gallery where one photo might be missing)

```javascript
const results = await Promise.allSettled(photos.map(p => loadPhoto(p)));
...
el.innerHTML = `<img alt="${escapeAttr(photoTitle)}">`;
```
...
// Custom events
document.dispatchEvent(new CustomEvent('cart:updated', { detail: { count: 3 } }));
...
```javascript
// Mailto URI (order submission) — use config.ORDER_EMAIL, never hardcode
import { config } from './site.config.js';
const mailto = `mailto:${config.ORDER_EMAIL}`
  + `?subject=${encodeURIComponent(config.ORDER_SUBJECT_PREFIX)}`
  + `&body=${encodeURIComponent('Order details...')}`;
...
// Replace 'x' with × in size strings
...
// e.g., 'gallery-abstract.html'

// Check if on a specific page
const isOrdering = window.location.pathname.includes('ordering');
