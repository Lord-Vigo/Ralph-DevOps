...
- **IOC status tracking**: Tracked in the parent spec file. (see `specs/master_spec.md` for breakdown and status).
...
5. **Fresh Karen per Loop**: Each time Karen is assigned to monitor, it must be a fresh spawn. This avoids context compaction and ensures consistent behavioral enforcement.
6. **Proactive Karen Monitoring**: Karen is always spawned alongside agents proactively, not just when issues arise. Karen works in real-time alongside each agent.
...
- Prune session.md to keep only the 3 most recent sessions (remove older session data)
...
3. **Use Knowledge Base**: Query Carl for relevant KB articles before implementing; ask Carl about best practices and tool usage.
...
| Visual verification | Build → run → screenshot → verify UI renders correctly |
...
5. **Verify Pass Criteria**: `cargo build`, `cargo test`, `cargo clippy`, no regression, visual verification, manual verification per IOC spec.
...
### Key Principles
- **Fresh Karen per cycle**: Each monitoring assignment uses a fresh Karen spawn to avoid context compaction.
- **Proactive monitoring**: Karen is always spawned alongside agents, not just when issues arise.
- **Active observation**: Karen works in real-time alongside each agent.
...
### Spawn Pattern
PM spawns agents in pairs for each cycle:
```
PM spawns FRESH Chad + FRESH Karen (Karen monitors Chad)
PM spawns FRESH Pat + FRESH Karen (Karen monitors Pat)
```
...
3. **Maintain Style Guide**: Keep `kb/nverse/ui_style_guide_kb.md` current with colors, typography, spacing, and component patterns.
...
Outputs: `specs/ui/design/<feature>_design.md`, `kb/<project>/ui_style_guide_kb.md`, `kb/<project>/ui_patterns_kb.md`. See `kb/hr/ab_tami_responsibilities_kb.md` for full detail.
...
└─────────────────────────┬───────────────────────────────────────────────┘
           ...
           ▼                              ▼
┌─────────────────────────┐    ┌─────────────────────────────────────────┐
│    [FRESH Chad]         │    │    [FRESH Karen]                       │
│  (implements IOC)       │    │  (monitors Chad's work)                │
│  reads Tami's spec      │    │  Corrective action if needed          │
│  spawns Carl for help   │    │                                        │
└───────────┬─────────────┘    └───────────────┬───────────────────────┘
            │                                  │
            │ Chad reports complete            │ Karen: deviation?
            └──────────────────┬──────────────┘
                               │
                               ▼
            ┌─────────────────────────────────────────┐
...
│                                                                         │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐ │
│  │ Build       │  │ Tests       │  │ Lint        │  │ Manual      │ │
│  │  build cmd  │  │  test cmd   │  │  lint cmd   │  │ Verification│ │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘ │
│         │                │                │                │         │
│         └────────────────┴────────────────┴────────────────┘         │
│                               │                                        │
...
- **Chad**: Check KB via Carl before implementing. Read the IOC spec; do not assume something is not implemented. Understand dependencies (`depc.md`). Implement one IOC at a time. Spawn Carl for tool/environment questions. For UI IOCs, read Tami's design spec first. Report completion to PM.
- **Pat + Karen**: Always spawned as a fresh pair. Karen monitors Pat in real-time. See `kb/hr/ab_pat_testing_kb.md` for the full testing flow.
